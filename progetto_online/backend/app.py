...
def get_db():
    if 'db' not in g:
        g.db = mysql.connector.connect(
            ...
            database=os.environ.get("MYSQL_DB", "gymnica")
        )
    return g.db
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS corsi_data (
        ...
        importo VARCHAR(50),
        UNIQUE KEY idx_corso_row_month (corso, row_index, mese)
    ) ENGINE=InnoDB;
    """)
    cur.execute("""
    CREATE TABLE IF NOT EXISTS course_totals (
        ...
        total_cassa DOUBLE DEFAULT 0,
        total_istruttore DOUBLE DEFAULT 0,
...
@app.route("/admin")
def admin():
    return send_from_directory(os.path.join(BASE_DIR, "..", "frontend", "templates"), "admin.html")
...
def check_ip_block(table_name, ip):
    ...
    # Recupera tentativi e ultimo tentativo
    ...
    if row:
        ...
        if tentativi >= MAX_ATTEMPTS:
            # Calcola i secondi trascorsi usando ora server
            cur.execute("SELECT TIMESTAMPDIFF(SECOND, %s, NOW()) AS elapsed", (last,))
            elapsed = cur.fetchone()["elapsed"]

            remaining = max(0, BLOCK_TIME_SECONDS - elapsed)
            if remaining > 0:
                return True, remaining
            else:
                # Reset tentativi e last_attempt
...
# ------------------------
# --- ROTTE ADMIN CORSI (MySQL) ---
# ------------------------
from datetime import datetime

def generate_months(start_month=10, start_year=2025, years_ahead=10):
...
def save_single_course_row(corso):
    ...
    if user:
        ...
        cur.execute(
            "INSERT INTO utenti (nome_cognome, email, phone) VALUES (%s, %s, %s)",
...
def save_course_totals(corso):
    ...
    cur.execute("""
        ...
        VALUES (%s,%s,%s,%s)
        ON DUPLICATE KEY UPDATE total_cassa=VALUES(total_cassa), total_istruttore=VALUES(total_istruttore)
